LDsentinalisation2 <-
  function(resids,regID,pathFinalSentinalised,pathUnsentinalised,FDRthr,my.covTMP,snpLocation,tmpFolder)
  {
...
LDsentExonExonJunc <-
  function(resids,regID,mapExon,pathFinalSentinalised,pathUnsentinalised,FDRthr,my.covTMP,snpLocation,tmpFolder)
  {
...
#     geneID <- unlist(sapply(strsplit(as.character(geneID),"+",fixed=T),function(x){x[1]}))
    ...
    geneID <- as.character(mapExon[mapExon$exonID %in% unlist(strsplit(as.character(regID),"_"))[1],"geneID"])
