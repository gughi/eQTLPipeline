...
splitSNPsByRegion <- function(i,chr,allMarkers,intergenicRegions,outputFolder,imputed.info,regIDsLogNo){
  ...
  fn.rda <- paste0(outputFolder,"byRegion/chr",chr,"/", i, ".rda")
  cmd <- paste0("awk '$1==\"", chr, "\" && $2 >=",
                start(intergenicRegions[i]) - 1000000,
                " && $2 <=", end(intergenicRegions[i]) + 1000000,
                " {print $3}' ",snps.map)
  ...
  if( length(vars) == 0 ){
    ...
    cat(i, chr, start(intergenicRegions[i]),end(intergenicRegions[i]), 0, 0, "\n",
        file=paste(outputFolder,"/logs/chr",chr,"/",i,".log", sep="")) 
    cat(i, "\n", file=paste0(regIDsLogNo,"chr",chr), append=T)
  ...
  cat(i, chr, start(intergenicRegions[i]),end(intergenicRegions[i]), sum(markers.info$type=="SNP"), sum(markers.info$type=="indel"), "\n",
      file=paste(outputFolder,"/logs/chr",chr,"/",i,".log", sep=""))
}
...
load(paste0("data/expr/rawCounts/intergenic/PUTM/chr",chr,".rda"))

outputFolder <- "data/snps/byRegion/PUTM/"
dir.create(paste0(outputFolder,"chr",chr), showWarnings=FALSE )
dir.create(paste0(outputFolder,"/logs"), showWarnings=FALSE)
dir.create(paste0(outputFolder,"/logs/chr",chr), showWarnings=FALSE)
...
regIDsLogNo <- "/home/seb/DERFINDER/eQTL/snps/tIDs_noPolys/"
